...
# Absolute path to the directory of this file (your blueprint file)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))

# Upload folder inside your app folder's static/uploads
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        # Status logic: Upcoming if future date, Ongoing if today or past but not done
        task_date = datetime.strptime(date, "%Y-%m-%d").date()
        today = datetime.today().date()
        status = "Upcoming" if task_date > today else "Ongoing"
...
def send_email(task_id, date):
    ...
    body = (
        ...
        f"Start: {task_data.get('start')}\n"
        f"End: {task_data.get('end')}\n"
                ...
                if user in date_tasks:
                    for task_id, task_data in date_tasks[user].items():
                        line = (
                            ...
                            f"({task_data.get('start')} - {task_data.get('end')}) | "
                            f"Status: {task_data.get('status')}"
                        )
