...
if not firebase_admin._apps:
    cred = credentials.Certificate('app\calendar-395f6-firebase-adminsdk-fbsvc-6c165f90f0.json')
...
def dashboard():
    ...
    try:
        ...
        flash(f"Error loading tasks from Firebase: {e}")
        tasks = []
...
@main.route('/mark_as_done/<int:task_index>', methods=['GET'])
def mark_as_done(task_index):
    ...
    try:
        ...
        for date, users in all_tasks.items():
            if user in users:
                for task_id, task_data in users[user].items():
                    if task_counter == task_index:
                        # Update the task status to Done
                        tasks_ref.child(f"{date}/{user}/{task_id}/status").set("Done")
...
def delete_task(task_index):
    ...
    try:
        ...
        for date, users in all_tasks.items():
            if user in users:
                for task_id in list(users[user].keys()):
                    if task_counter == task_index:
                        # Delete task
                        tasks_ref.child(f"{date}/{user}/{task_id}").delete()
                        flash("Task deleted!")
                        return redirect(url_for('main.dashboard'))
                    task_counter += 1
